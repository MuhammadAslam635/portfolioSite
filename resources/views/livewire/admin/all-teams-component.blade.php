<div class="min-h-screen p-4">
                ...
                <div
                    ...
                    <div class="absolute top-4 left-4 z-10">
                        <div class="flex flex-row gap-1">
                            <flux:badge color="{{ $team->is_active ? 'lime' : 'red' }}"
                            ...
                            <flux:badge color="pink" class="cursor-pointer text-xs font-semibold animate-pulse"
                                wire:click="openModal({{ $team->id }})">Add Project / Skill</flux:badge>
                        </div>
                    </div>
                        ...
                        <div class="absolute bottom-4 left-4">
                            <div class="flex flex-row gap-1">
                                <flux:button variant="primary" color="indigo" size="sm" icon="pencil"
                                    ...
                                    :href="route('team.edit',['slug'=>$team->slug])" wire:navigate>
                                    Edit
                                </flux:button>
    ...
    <flux:modal wire:model.self="showConfirmModal" variant="flyout">
        <div class="space-y-6">
            ...
            <div class="flx rounded-md bg-green-50 gap-1">
                <form wire:submit.prevent="addSkill" class="space-y-4 p-2">
                    <flux:select wire:model="teamSkill" placeholder="Choose Skill...">
                        @foreach ($skills as $skill)
                            <flux:select.option value="{{ $skill->id }}">{{ $skill->name }}</flux:select.option>
                        @endforeach
                    </flux:select>

                    <flux:input label="Skil Score" type="number" wire:model="teamSkillScore" />
        ...
        <div class="space-y-6">
            ...
            <div class="flx rounded-md bg-green-50 gap-1">
                <form wire:submit.prevent="addProject" class="space-y-4 p-2">
                    <flux:select wire:model="projectId" placeholder="Choose Project...">
                    ...
                    <flux:select wire:model="role" placeholder="Choose Role...">
                            ...
                            <flux:select.option value="2">{{ "Developer" }}</flux:select.option>
